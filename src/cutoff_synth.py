...
class CutoffModule(nn.Module):
    def __init__(self):
        super().__init__()
        self.alphas = nn.Parameter(torch.randn(2))
        self.bias = nn.Parameter(torch.randn(1))
        self.lamb = nn.Parameter(torch.randn(1))

    def forward(self, x):
        a = torch.sum(F.gumbel_softmax(self.alphas, tau=1, hard=True) * torch.tensor([-1, 1]).cuda())
        return F.sigmoid(self.lamb) * x + (1 - F.sigmoid(self.lamb)) * (a * F.relu(a * (x - self.bias)) + self.bias)



class CutoffMLP(nn.Module):
    def __init__(self):
        super().__init__()
        self.layers = nn.Sequential(
            CutoffModule(),
...
use_learnable_cutoff = False

num_train_data = 100000
num_valid_data, num_test_data = 10000, 10000
num_repeats = 1
epochs = 50
...
# experiment 5 (learnable cutoff module)

# cutoff = 'ReLU' # 'ReLU', None
# use_cutoffs = [True]
# use_learnable_cutoff = True
# log_dir = "experiments/learnable_cutoff"

# experiment 6 (decision tree)
gen_models = [gen_data2(), gen_data3()]
use_decision_tree = True
log_dir = "experiments/decision_tree"
...
for task in tasks:
    ...
    for gen_model in gen_models:
        for num_repeat in range(num_repeats):
            input_feat = np.random.normal(size=(num_train_data + num_valid_data + num_test_data, 10))
            ...
            for use_cutoff in use_cutoffs:
                ...
                logger.info(f"task, model, num_repeat, use_cutoff: {task}, {type(gen_model)}, {num_repeat}, {use_cutoff}")
                ...
                if use_decision_tree:
                    ...
                    for c in range(10):
                        threshold_list = []
                        decision_tree_regressor = decision_tree_regressor.fit(input_feat[:,c,np.newaxis], s1)

                        n_nodes = decision_tree_regressor.tree_.node_count
                        children_left = decision_tree_regressor.tree_.children_left
                        children_right = decision_tree_regressor.tree_.children_right
                        feature = decision_tree_regressor.tree_.feature
                        threshold = decision_tree_regressor.tree_.threshold
                        ...
                        for i in range(n_nodes):
                            if not is_leaves[i]:
                                threshold_list.append(threshold[i])
                        threshold_list = sorted(threshold_list)
                        logger.info(f"c-th threshold_list: {c}-th {threshold_list}")

                        new_input_feat_col = []
                        for input_feat_col in input_feat[:,c,np.newaxis]:
                            if input_feat_col < threshold_list[0]:
                                new_input_feat_col.append([1, 0, 0, 0])
                            elif threshold_list[0] <= input_feat_col < threshold_list[1]:
                                new_input_feat_col.append([0, 1, 0, 0])
                            elif threshold_list[1] <= input_feat_col < threshold_list[2]:
                                new_input_feat_col.append([0, 0, 1, 0])
                            else:
                                new_input_feat_col.append([0, 0, 0, 1])
                        new_input_feat.append(new_input_feat_col)
                    new_input_feat = np.concatenate(new_input_feat, axis=-1)
                    input_feat = new_input_feat
                ...
                T1_train, T1_test, T1_train_y, T1_test_y = train_test_split(T1.iloc[:, :-1], T1.iloc[:, -1], test_size=num_valid_data+num_test_data, random_state=42)
                T1_valid, T1_test, T1_valid_y, T1_test_y = train_test_split(T1_test, T1_test_y, test_size=num_test_data, random_state=42)
                ...
                if use_learnable_cutoff:
                    model = CutoffMLP()
                elif use_decision_tree:
                    model = MLP(input_dim=40)
                else:
                ...
                for m in model.modules():
                    if isinstance(m, CutoffModule):
                        print(f"m.alphas: {F.softmax(m.alphas)}")
                        print(f"m.bias: {m.bias}")
                        print(f"m.lamb: {F.sigmoid(m.lamb)}")
                ...
                if task == 'regression':
                    ...
                    accuracy = test_corrected / test_len
