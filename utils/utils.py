...
def js_divergence(p1, p2):
    total_m = 0.5 * (p1 + p2)
    loss = 0.5 * F.kl_div(
        torch.log(p1), total_m, reduction="batchmean"
    ) + 0.5 * F.kl_div(torch.log(p2), total_m, reduction="batchmean")
    return loss
...
def get_feature_importance(args, dataset, test_data, test_mask, model):
    ...
    feature_importance = feature_importance / torch.sum(
        feature_importance, dim=-1, keepdim=True
    )
...
def get_mask_by_feature_importance(args, dataset, test_data, importance):
    ...
    mask = np.take_along_axis(
        np.concatenate(
            [
                np.ones((test_data.shape[0], len_keep)),
                np.zeros((test_data.shape[0], input_dim - len_keep)),
            ],
            axis=1,
        ),
        idx,
    ...
    if dataset.cont_dim and len(dataset.cat_indices_groups):
        ...
        cat_mask = np.concatenate(
            [
                np.repeat(
                    mask[:, dataset.cont_dim :][:, category_idx][:, None],
                    len(category),
                    axis=1,
                )
                for category_idx, category in enumerate(
                    dataset.input_one_hot_encoder.categories_
                )
            ],
            axis=1,
        )
        mask = torch.FloatTensor(np.concatenate([cont_mask, cat_mask], axis=-1)).to(
            test_data.device
        )
        ...
        mask = torch.FloatTensor(
            np.concatenate(
                [
                    np.repeat(cat_mask[:, category_idx][:, None], len(category), axis=1)
                    for category_idx, category in enumerate(
                        dataset.input_one_hot_encoder.categories_
                    )
                ],
...
def draw_histogram(args, x_list, title, xlabel, ylabel):
    with open(
        file=f"pickle/{args.dataset}_{args.model}_{args.seed}_ent.pickle", mode="wb"
    ) as f:
...
def draw_tsne(args, feats, cls, title):
    ...
    with open(
        file=f"pickle/{args.dataset}_{args.model}_{args.seed}_tsne.pickle", mode="wb"
    ) as f:
        pickle.dump(z, f)
        print(f"pickle/{args.dataset}_{args.model}_{args.seed}_tsne.pickle is saved!")
    with open(
        file=f"pickle/{args.dataset}_{args.model}_{args.seed}_tsne_cls.pickle",
        mode="wb",
    ) as f:
        pickle.dump(cls, f)
        print(
            f"pickle/{args.dataset}_{args.model}_{args.seed}_tsne_cls.pickle is saved!"
        )
...
def draw_reliability_plot(args, confs, preds, labels, title, num_bins=10):
    ...
    with open(
        file=f"pickle/calibration_bns_{args.dataset}_{args.model}_{args.seed}.pickle",
        mode="wb",
    ) as f:
        pickle.dump(bns, f)

    with open(
        file=f"pickle/calibration_y_{args.dataset}_{args.model}_{args.seed}.pickle",
        mode="wb",
    ) as f:
        pickle.dump(y, f)
    ...
    plt.show()
    plt.savefig(
        f"{args.benchmark}_{args.dataset}_{args.shift_type}_{args.shift_severity}_{args.model}_{'_'.join(args.method)}_{title}.png"
    )
