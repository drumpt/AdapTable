...
def pretrain(args, model, optimizer, dataset):
    ...
    for epoch in range(1, args.pretrain_epochs + 1):
        ...
        for train_x, _ in dataset.train_loader:
            ...
            estimated_x = model(train_cor_x) if isinstance(model(train_cor_x), torch.Tensor) else model(train_cor_x)[0]
        ...
        with torch.no_grad():
            for valid_x, _ in dataset.valid_loader:
                ...
                estimated_x = model(valid_cor_x) if isinstance(model(train_cor_x), torch.Tensor) else model(valid_cor_x)[0]
...
def train(args, model, optimizer, dataset):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for train_x, train_y in dataset.train_loader:
            ...
            estimated_y = model(train_x) if isinstance(model(train_x), torch.Tensor) else model(train_x)[-1]
        ...
        with torch.no_grad():
            for valid_x, valid_y in dataset.valid_loader:
                ...
                estimated_y = model(valid_x) if isinstance(model(valid_x), torch.Tensor) else model(valid_x)[-1]
...
def forward_and_adapt(args, x, model, optimizer):
    ...
    optimizer.zero_grad()
    outputs = model(x) if isinstance(model(x), torch.Tensor) else model(x)[-1]

    if 'tent' in args.method:
        loss = softmax_entropy(outputs / args.temp).mean()
        loss.backward(retain_graph=True)
    if 'memo' in args.method:
        x = utils.generate_augmentation(x, args)
        outputs = model(x) if isinstance(model(x), torch.Tensor) else model(x)[-1]
        loss = softmax_entropy(outputs / args.temp).mean()
        loss.backward(retain_graph=True)
    if 'sar' in args.method:
        ...
        new_outputs = model(x) if isinstance(model(x), torch.Tensor) else model(x)[-1]
...
def main_em(args):
    ...
    ORIGINAL_FEATURE_LIST_SOURCE, ORIGINAL_FEATURE_LIST_TARGET, ADAPTED_FEATURE_LIST_SOURCE, ADAPTED_FEATURE_LIST_TARGET, TARGET_LABEL = [], [], [], [], []
    EMA = None
    ...
    for test_x, _, test_y in original_dataset.test_loader:
        ...
        TARGET_LABEL.extend(torch.argmax(test_y, dim=-1).cpu().detach().numpy().tolist())
    ...
    X_embedded = TSNE(n_components=2, learning_rate='auto', init='random').fit_transform(total_features)
    ...
    df = pd.DataFrame()
    df["y"] = TARGET_LABEL
    df["d1"] = ORIGINAL_FEATURE_LIST_SOURCE[:, 0]
    df["d2"] = ORIGINAL_FEATURE_LIST_SOURCE[:, 1]
    sp = sns.scatterplot(x="d1", y="d2", hue=df.y.tolist(), palette=sns.color_palette("husl", test_y.shape[-1]), marker="p", data=df)
    sp.set(xlabel=None)
    sp.set(ylabel=None)

    df = pd.DataFrame()
    df["y"] = TARGET_LABEL
    df["d1"] = ORIGINAL_FEATURE_LIST_TARGET[:, 0]
    df["d2"] = ORIGINAL_FEATURE_LIST_TARGET[:, 1]
    sp = sns.scatterplot(x="d1", y="d2", hue=df.y.tolist(), palette=sns.color_palette("husl", test_y.shape[-1]), marker="x", data=df)
    sp.set(xlabel=None)
    sp.set(ylabel=None)
    ...
    df = pd.DataFrame()
    df["y"] = TARGET_LABEL
    df["d1"] = ADAPTED_FEATURE_LIST_SOURCE[:, 0]
    df["d2"] = ADAPTED_FEATURE_LIST_SOURCE[:, 1]
    sp = sns.scatterplot(x="d1", y="d2", hue=df.y.tolist(), palette=sns.color_palette("husl", test_y.shape[-1]), marker="p", data=df)
    sp.set(xlabel=None)
    sp.set(ylabel=None)
    ...
    df = pd.DataFrame()
    df["y"] = TARGET_LABEL
    df["d1"] = ADAPTED_FEATURE_LIST_TARGET[:, 0]
    df["d2"] = ADAPTED_FEATURE_LIST_TARGET[:, 1]
    sp = sns.scatterplot(x="d1", y="d2", hue=df.y.tolist(), palette=sns.color_palette("husl", test_y.shape[-1]), marker="x", data=df)
    sp.set(xlabel=None)
    sp.set(ylabel=None)
...
def main_mae(args):
    ...
    for test_x, test_mask_x, test_y in dataset.test_loader:
        ...
        test_cor_x, test_cor_mask_x = get_corrupted_data(test_x, dataset.dataset.train_x, data_type="numerical", shift_type="random_drop", shift_severity=args.test_mask_ratio, imputation_method="emd")
        ...
        if not args.no_mae_based_imputation:
            print("implement mae based imputation!")
    ...
    df["d1"] = ORIGINAL_FEATURE_LIST_SOURCE[:, 0]
    df["d2"] = ORIGINAL_FEATURE_LIST_SOURCE[:, 1]
    sns.scatterplot(x="d1", y="d2", hue=df.y.tolist(), palette=sns.color_palette("husl", test_y.shape[-1]), marker="p", data=df)

    df = pd.DataFrame()
    df["y"] = TARGET_LABEL
    df["d1"] = ORIGINAL_FEATURE_LIST_TARGET[:, 0]
    df["d2"] = ORIGINAL_FEATURE_LIST_TARGET[:, 1]
    sns.scatterplot(x="d1", y="d2", hue=df.y.tolist(), palette=sns.color_palette("husl", test_y.shape[-1]), marker="x", data=df)
    ...
    plt.legend(ncol=test_y.shape[-1])

    plt.title("After Adaptation")
    plt.savefig("tsne_after_adaptation_mae.png")

    print(f"We are in main_mae!")
