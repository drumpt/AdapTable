...
def plot_feature_output(args, dataset, source_model, test_model):
    ...
    src_shap_importance, _ = get_shap_values(
        dataset,
        source_model,
        torch.tensor(dataset.train_x).float().to(args.device),
        args,
    )
    src_shap_importance_test, _ = get_shap_values(
        dataset,
        source_model,
        torch.tensor(dataset.train_x).float().to(args.device),
        args,
    ...
    test_shap_importance, _ = get_shap_values(
        dataset, test_model, torch.tensor(dataset.test_x).float().to(args.device), args
    )
    test_shap_importance_test, _ = get_shap_values(
        dataset, test_model, torch.tensor(dataset.train_x).float().to(args.device), args
    )
...
def plot_learned_bias(args, model, dataset):
    ...
    for epoch in range(50):
        optimizer.zero_grad()
        biased_output = model(
            torch.tensor(dataset.train_x).float().to(args.device) + bias
        )
        loss = F.cross_entropy(
            biased_output,
            torch.argmax(torch.tensor(dataset.train_y).float().to(args.device), dim=-1),
        )
        loss.backward()
        optimizer.step()
        print(loss.item())
    ...
    bias_test = torch.zeros_like(
        torch.tensor(dataset.train_x[0]).float().to(args.device)
    )
    ...
    for epoch in range(50):
        optimizer.zero_grad()
        biased_output = model(
            torch.tensor(dataset.test_x).float().to(args.device) + bias_test
        )
        loss = F.cross_entropy(
            biased_output,
            torch.argmax(torch.tensor(dataset.test_y).float().to(args.device), dim=-1),
        )
        loss.backward()
        optimizer.step()
        print(loss.item())
    ...
    plt.subplot(3, 1, 3)
    plt.bar(
        range(len(bias_test)), np.clip(np.abs(bias_test - bias) / np.abs(bias), 0, 50)
    )
    plt.xlabel("Index")
    plt.ylabel("Value")
...
def plot_recon_feature(args, dataset, model):
    ...
    shap_importance, shap_explainer = get_shap_values(
        dataset, model, torch.tensor(dataset.train_x).float().to(args.device), args
    )

    print(shap_importance)

    idx_unimportant = np.argsort(shap_importance, axis=-1)[
        : int(len(shap_importance) * 0.25)
    ]
    ...
    model = model.to(args.device)
    train_feature = (
        model.get_feature(model.get_recon_out(torch_train_x)).cpu().detach().numpy()
    )
    test_feature = (
        model.get_feature(model.get_recon_out(torch_test_x)).cpu().detach().numpy()
    )
    ...
    for label in np.unique(train_y):
        idx = train_y == label
        plt.scatter(
            train_tsne_results[idx, 0],
            train_tsne_results[idx, 1],
            label=f"Label {label}",
        )
    plt.title("Train Features")
    ...
    for label in np.unique(test_y):
        idx = test_y == label
        plt.scatter(
            test_tsne_results[idx, 0], test_tsne_results[idx, 1], label=f"Label {label}"
        )
    plt.title("Test Features")
    plt.xlim(x_min, x_max)
    plt.ylim(y_min, y_max)
...
def plot_acc_over_relative_entropy(args, dataset, model):
    ...
    def compute_relative_entropy(predictions, num_classes, temperature=1.0):
        ...
        entropy = -np.sum(
            softmax_output * np.log(softmax_output + 1e-10), axis=1
        )  # Add a small constant to avoid log(0)
        relative_entropy = entropy / np.log(num_classes)
        return relative_entropy

    def plot_accuracy_vs_binned_entropy(
        predictions, true_labels, bin_count, temperature=1.0
    ):
        import numpy as np
        import matplotlib.pyplot as plt
        ...
        def plot_accuracies_and_label_distribution(
            relative_entropies, accuracies, labels, bin_count
        ):
            ...
            bin_edges = np.quantile(
                relative_entropies, q=np.linspace(0, 1, bin_count + 1)
            )
            ...
            for idx, label in enumerate(unique_labels):
                ...
                label_distribution = [
                    np.sum(label_mask[bin_indices == bin_idx])
                    / np.sum(bin_indices == bin_idx)
        ...
        def process_bins(predictions, labels, temperatures, bin_count):
            ...
            for i in range(bin_count):
                logits[bin_indices == i] = apply_temperature_scaling(
                    logits[bin_indices == i], temperatures[i]
                )
            new_predictions = scipy.special.softmax(logits, axis=1)
            new_relative_entropies = compute_relative_entropy(
                new_predictions, num_classes
            )

            # 4. Compute each bin's accuracies
            accuracies = [
                np.mean(
                    labels[bin_indices == i]
                    == np.argmax(new_predictions[bin_indices == i], axis=1)
                )
                for i in range(bin_count)
            ]

            # 5. Plot accuracies and label distribution
            plot_accuracies_and_label_distribution(
                new_relative_entropies, accuracies, labels, bin_count
            )
    ...
    def plot_accuracy_vs_relative_entropy(
        predictions, true_labels, bin_count, temperature=1.0
    ):
        ...
        relative_entropies = compute_relative_entropy(
            predictions, num_classes, temperature=temperature
        )
    ...
    plot_accuracy_vs_relative_entropy(
        predictions_test, true_labels_test, bin_count=10, temperature=0.8
    )
    plot_accuracy_vs_binned_entropy(
        predictions_test, true_labels_test, bin_count=10, temperature=0.8
    )
...
def plot_stunt_tasks(args, dataset, model):
    ...
    train_x, train_y = torch.tensor(dataset.train_x).to(args.device), torch.tensor(
        dataset.train_y
    ).to(args.device)
    test_x, test_y = torch.tensor(dataset.test_x).to(args.device), torch.tensor(
        dataset.test_y
    ).to(args.device)

    emd_imputed_train_x = Dataset.get_imputed_data(
        train_x, dataset.train_x, data_type="numerical", imputation_method="zero"
    )
    emd_imputed_test_x = Dataset.get_imputed_data(
        test_x, dataset.train_x, data_type="numerical", imputation_method="zero"
    )
    model = model.to(args.device)

    for important_idx in range(train_x.shape[1]):

        train_prototype_x, train_prototype_y, train_prototype_target = (
            StuntPrototype.construct_label_and_input(
                model, train_x, emd_imputed_train_x, important_idx, num_bins=10
            )
        )
        test_prototype_x, test_prototype_y, test_prototype_target = (
            StuntPrototype.construct_label_and_input(
                model, test_x, emd_imputed_test_x, important_idx, num_bins=10
            )
        )
...
def plot_maskratio(args, dataset, model):
    ...
    for label in np.unique(all_labels):
        ...
        for gt_label in np.unique(mask_ratio_y):
            ...
            plt.scatter(
                mask_ratio_x[idx_inner, 0],
                mask_ratio_x[idx_inner, 1],
                label=f"Label {gt_label}",
            )
            plt.xlim(x_min, x_max)
            plt.ylim(y_min, y_max)
...
def pooled_avg_plot(args, model, dataset):
    ...
    for data in [(train_x, train_y), (test_x, test_y)]:
        ...
        for label in range(dataset.train_y.shape[1]):
            idx = y == label
            plt.scatter(
                do_tsne_feature[idx, 0], do_tsne_feature[idx, 1], label=f"Label {label}"
            )
            plt.xlim(x_min, x_max)
            plt.ylim(y_min, y_max)
        plt.legend()
...
def plot_recon_loss_per_column_per_class(args, model, dataset):
    ...
    train_x, train_y = torch.tensor(dataset.train_x).float().to(
        args.device
    ), torch.tensor(dataset.train_y).float().to(args.device)
    test_x, test_y = torch.tensor(dataset.test_x).float().to(args.device), torch.tensor(
        dataset.test_y
...
def plot_recon_loss_per_column(args, model, dataset):
    ...
    def kl_divergence(h1, h2):
        ...
        return max(
            np.sum(
                h1_rel_freq[nonzero_idx]
                * np.log(h1_rel_freq[nonzero_idx] / h2_rel_freq[nonzero_idx])
            ),
            0,
        )
    ...
    def js_distance(h1, h2):
        ...
        kl_div1 = np.sum(
            h1_rel_freq[nonzero_idx1]
            * np.log(
                (h1_rel_freq[nonzero_idx1] + np.finfo(float).eps)
                / (m[nonzero_idx1] + np.finfo(float).eps)
            )
        )

        kl_div2 = np.sum(
            h2_rel_freq[nonzero_idx2]
            * np.log(
                (h2_rel_freq[nonzero_idx2] + np.finfo(float).eps)
    ...
    def calculate_columnwise_kl_divergence(train_data, test_data, bins=1000):
        ...
        for i in range(num_columns):
            ...
            train_hist, bin_edges = np.histogram(
                train_data[:, i], bins=bins, density=True, range=(min, max)
            )
            test_hist, _ = np.histogram(
                test_data[:, i], bins=bin_edges, density=True, range=(min, max)
            )
    ...
    kl_divergences = calculate_columnwise_kl_divergence(
        dataset.train_x, dataset.test_x, bins=200
    )
    ...
    plt.subplot(3, 1, 2)
    plt.bar(
        range(len(recon_loss_per_column_test)),
        np.abs(recon_loss_per_column_test - recon_loss_per_column),
    )
    plt.xlabel("Index")
    plt.ylabel("Value")
    ...
    mask_vector = (mask_vector - np.min(mask_vector)) / (
        np.max(mask_vector) - np.min(mask_vector)
    )

    plt.subplot(3, 1, 3)
    bars = plt.bar(
        range(len(recon_loss_per_column_test)),
        recon_loss_per_column_test - recon_loss_per_column,
    )
    plt.xlabel("Index")
    plt.ylabel("Value")
...
def linear_layer_exp(args, model, dataset):
    ...
    linear_layer = nn.Sequential(nn.Linear(128, 2), nn.Softmax(dim=-1))
    ...
    upper_half = (
        torch.tensor(np.argsort(shap_importance)[len(shap_importance) // 4 :])
        .long()
        .to(args.device)
    )
    lower_half = (
    ...
    for epoch in range(5):
        x = test_x
        y = test_y
        emd_imputed_x = Dataset.get_imputed_data(
            x, dataset.train_x, data_type="numerical", imputation_method="zero"
        )
        from utils.stunt_prototype import StuntPrototype
        ...
        with torch.no_grad():
            ...
            loss_student_b = F.kl_div(
                (student_a + 1e-6).log(), student_b, reduction="batchmean"
            )

            print(
                "losses are: {}, {}, {}".format(
                    loss_ce.item(), loss_student_a.item(), loss_student_b.item()
                )
            )
...
def prediction_based_on_mask(args, model, dataset):
    ...
    class_prototypes_train_np = [
        class_prototypes_train[0].detach().cpu().numpy(),
        class_prototypes_train[1].detach().cpu().numpy(),
    ]
    class_prototypes_test_np = [
        class_prototypes_test[0].detach().cpu().numpy(),
        class_prototypes_test[1].detach().cpu().numpy(),
    ]
    tsne = TSNE(n_components=2, random_state=0, perplexity=2)
    X_2d = tsne.fit_transform(
        np.array(
            [
                class_prototypes_train_np[0],
                class_prototypes_train_np[1],
                class_prototypes_test_np[0],
                class_prototypes_test_np[1],
            ]
        )
    )
    ...
    for idx in range(len(dataset.test_x)):
        ...
        norm_mask_vector_zero = (mask_vector_zero - torch.min(mask_vector_zero)) / (
            torch.max(mask_vector_zero) - torch.min(mask_vector_zero)
        )
        norm_mask_vector_one = (mask_vector_one - torch.min(mask_vector_one)) / (
            torch.max(mask_vector_one) - torch.min(mask_vector_one)
        )
        ...
        shap_indices = np.argsort(shap_importance)[::-1][
            : int(len(shap_importance) * 0.5)
        ]
        mask_indices_zero = np.argsort(norm_mask_vector_zero)[::-1][
            : int(len(shap_importance) * 0.1)
        ]
        mask_indices_one = np.argsort(norm_mask_vector_one)[::-1][
            : int(len(shap_importance) * 0.1)
        ]

        intersection_zero = len(
            set(shap_indices).intersection(set(mask_indices_zero))
        ) / len(set(shap_indices).union(set(mask_indices_zero)))
        intersection_one = len(
            set(shap_indices).intersection(set(mask_indices_one))
        ) / len(set(shap_indices).union(set(mask_indices_one)))
...
def interpolation(args, model, dataset):
    ...
    shap_importance = np.mean(np.abs(shap_importance), axis=0)
    all = torch.tensor(
        list(np.argsort(shap_importance)[::-1][: int(len(shap_importance) * 1)])
    ).long()
    upper25 = torch.tensor(
        list(np.argsort(shap_importance)[::-1][: int(len(shap_importance) * 0.3)])
    ).long()
    lower25 = torch.tensor(
        list(np.argsort(shap_importance)[: int(len(shap_importance) * 0.3)])
    ).long()
    emd_imputed_test_x = Dataset.get_imputed_data(
        test_x, dataset.train_x, data_type="numerical", imputation_method="emd"
    )
    emd_imputed_train_x = Dataset.get_imputed_data(
        train_x, dataset.train_x, data_type="numerical", imputation_method="emd"
    )
    ...
    for idx in [all, upper25, lower25]:
        ...
        masked_feature = masked_feature / np.linalg.norm(
            masked_feature, axis=-1, keepdims=True
        )
        ...
        plt.scatter(
            masked_feature[:, 0],
            masked_feature[:, 1],
            c=np.argmax(train_y.cpu().detach().numpy(), axis=-1),
        )
        plt.legend()
        plt.show()
    ...
    for idx in upper25:
        ...
        plt.hist(
            train_x[:, idx].cpu().detach().numpy(), bins=100, alpha=0.5, label="train"
        )
        plt.title(f"feature {idx}, train")
        plt.show()
        plt.hist(
            test_x[:, idx].cpu().detach().numpy(), bins=100, alpha=0.5, label="test"
        )
        plt.title(f"feature {idx}, test")
        plt.show()
        plt.hist(
            recon_x[:, idx].cpu().detach().numpy(), bins=100, alpha=0.5, label="test"
        )
...
def cont_cat_features(args, model, dataset):
    ...
    tsne = TSNE(n_components=2, random_state=0)
    feature_contcat = tsne.fit_transform(
        np.concatenate([feature_cont, feature_cat], axis=0)
    )
    feature_contcat_cmap = np.concatenate(
        [np.zeros_like(feature_cont[:, 0]), np.ones_like(feature_cat[:, 0])], axis=0
    )
    feature_contcat_labelcmap = np.concatenate(
        [np.argmax(dataset.train_y, axis=-1), np.argmax(dataset.train_y, axis=-1)],
        axis=0,
    )
    # get recon_x
...
def conv_features(args, dataset, model):
    ...
    for epoch in range(100):
        ...
        loss = (
            F.mse_loss(initial_feature, upper_feature)
            + F.mse_loss(initial_feature, lower_feature)
            + F.mse_loss(initial_feature_shallow, upper_feature_shallow)
        )
    ...
    with torch.no_grad():
        ...
        plt.scatter(
            feature_tsne[:, 0],
            feature_tsne[:, 1],
            c=np.argmax(test_y.cpu().detach().numpy(), axis=-1),
            cmap="rainbow",
        )
        plt.show()
...
def plot_permutation_result(args, dataset, model):
    ...
    for epoch in range(100):
        unique_permutations = {
            tuple(perm): i
            for i, perm in enumerate(itertools.permutations(range(args.num_chunks)))
        }
        from utils.permutation import Permutation_Generation

        permute_train_x, permute_train_y = (
            Permutation_Generation.permutation_task_generation(
                train_x,
                unique_permutations,
                args.num_chunks,
        ...
        permute_test_x, permute_test_y = (
            Permutation_Generation.permutation_task_generation(
                test_x, unique_permutations, args.num_chunks, args.num_chunks_per_sample
            )
        )

        permute_train_x, permute_train_y = permute_train_x.to(
            args.device
        ).float(), permute_train_y.to(args.device)
        permute_test_x, permute_test_y = permute_test_x.to(
            args.device
        ).float(), permute_test_y.to(args.device)

        optimizer.zero_grad()
        (
            F.cross_entropy(model.get_ordering(permute_test_x), permute_test_y).mean()
        ).backward()
        optimizer.step()
        with torch.no_grad():
            ...
            train_acc = (
                (
                    torch.argmax(model.get_ordering(permute_train_x), dim=-1)
                    == permute_train_y
                )
                .float()
                .mean()
            ...
            test_acc = (
                (
                    torch.argmax(model.get_ordering(permute_test_x), dim=-1)
                    == permute_test_y
                )
                .float()
                .mean()
...
def permutation_invariant(args, dataset, model):
    ...
    with torch.no_grad():
        ...
        test_x, test_y = torch.tensor(dataset.test_x).float().to(
            args.device
        ), torch.tensor(dataset.test_y).float().to(args.device)
        ...
        train_acc = (
            (torch.argmax(model(train_x), dim=-1) == torch.argmax(train_y, dim=-1))
            .float()
            .mean()
            .item()
        )
        test_acc = (
            (torch.argmax(model(test_x), dim=-1) == torch.argmax(test_y, dim=-1))
            .float()
            .mean()
            .item()
        )
        ...
        train_acc = (
            (
                torch.argmax(model(train_x_permuted), dim=-1)
                == torch.argmax(train_y, dim=-1)
            )
            .float()
            .mean()
            .item()
        ...
        test_acc = (
            (
                torch.argmax(model(test_x_permuted), dim=-1)
                == torch.argmax(test_y, dim=-1)
            )
            .float()
            .mean()
            .item()
...
def permuation_invariant(args, dataset, model):
    ...
    for epoch in range(3):
        ...
        with torch.no_grad():
            ...
            print(
                "acc : ",
                (torch.argmax(estim_y, dim=-1) == torch.argmax(test_y, dim=-1))
                .float()
                .mean()
                .item(),
            )
...
def plot_gumbel_softmax(args, dataset, model):
    ...
    for epoch in range(10):
        ...
        with torch.no_grad():
            ...
            plt.bar(
                np.arange(len(mask_layer().detach().cpu().numpy())),
                mask_layer().detach().cpu().numpy(),
            )
            plt.show()
...
def use_avg_feature(args, dataset, model):
    ...
    for epoch in range(total_epochs):
        ...
        with torch.no_grad():
            ...
            mse_loss_list = torch.sum(
                crit(masked_output_feature * (~mask), test_x * (~mask)), dim=-1
            )
            mse_idx = torch.argsort(mse_loss_list, descending=True)[: len(test_x) // 10]

        optimizer.zero_grad()
        F.mse_loss(
            masked_output_feature[mse_idx] * (~mask[mse_idx]),
            test_x[mse_idx] * (~mask[mse_idx]),
        ).backward()
        optimizer.step()
        ...
        with torch.no_grad():
            ...
            # vanilla accuracy
            print(
                f"epoch : {epoch} vanilla accuracy : ",
                (torch.argmax(vanilla_output, dim=-1) == torch.argmax(test_y, dim=-1))
                .float()
                .mean()
                .item(),
            )
            print(
                f"epoch : {epoch} averaged accuracy : ",
                (torch.argmax(averaged_output, dim=-1) == torch.argmax(test_y, dim=-1))
                .float()
                .mean()
                .item(),
            )
...
def smotish(args, dataset, model):
    ...
    smote = SMOTE()
    upsampled_x, upsampled_group_labels = smote.fit_resample(
        test_x.detach().cpu().numpy(), group_labels
    )
...
def plot_shap_per_column(args, dataset, source_model, test_model):
    ...
    shap_importance_train, _ = get_shap_values(
        dataset,
        source_model,
        torch.tensor(dataset.train_x).float().to(args.device),
        args,
    ...
    shap_importance_test, _ = get_shap_values(
        dataset, test_model, torch.tensor(dataset.test_x).float().to(args.device), args
    )
...
def give_adaptive_recon_loss(args, dataset, source_model, test_model):
    ...
    split_idx_tensor_list = torch.split(
        idx_tensor_list, int(len(recon_loss) * 1 / buckets)
    )
    ...
    for epoch in range(300):
        ...
        low_recon_loss_columns = sorted_column_via_recon_loss[
            int(len(recon_loss) * 0.1) :
        ]  # Set an appropriate threshold

        # Step 2: Finding the Nearest Neighbor
        from sklearn.neighbors import NearestNeighbors

        nearest_neighbors = NearestNeighbors(n_neighbors=2, algorithm="ball_tree").fit(
            dataset.test_x
        )
        ...
        for i, test_instance in enumerate(test_x):
            nearest_neighbor = test_x[
                indices[i, 1]
            ]  # Getting the 2nd nearest neighbor because the nearest would be the instance itself
        ...
        loss = (
            F.cross_entropy(out, torch.softmax(out / 0.8, dim=1).detach()).mean()
            + F.cross_entropy(
                out_aug, torch.softmax(out_aug / 0.8, dim=1).detach()
            ).mean()
        )
        loss.backward()
        ...
        if epoch % 1 == 0:
            ...
            with torch.no_grad():
                ...
                test_acc = (
                    (torch.argmax(test_out, dim=-1) == torch.argmax(test_y, dim=-1))
                    .float()
                    .mean()
                )
                aug_test_acc = (
                    (torch.argmax(aug_test_out, dim=-1) == torch.argmax(test_y, dim=-1))
                    .float()
                    .mean()
                )
                ...
                print(
                    f"count of elems are: {np.unique(torch.argmax(test_out, dim=-1).detach().cpu().numpy(), return_counts=True)}"
                )
                print(
                    f"count of augelems are: {np.unique(torch.argmax(aug_test_out, dim=-1).detach().cpu().numpy(), return_counts=True)}"
                )
                print("\n")
            source_model.train()
...
def subtract_emd(args, model, dataset):
    ...
    emd_imputed_train_x = Dataset.get_imputed_data(
        train_x, dataset.train_x, data_type="numerical", imputation_method="mean"
    )
    emd_imputed_train_x = torch.tensor(emd_imputed_train_x).float().to(args.device)
    emd_imputed_test_x = Dataset.get_imputed_data(
        test_x, dataset.test_x, data_type="numerical", imputation_method="mean"
    )
    emd_imputed_test_x = torch.tensor(emd_imputed_test_x).float().to(args.device)
    ...
    for epoch in range(300):
        ...
        with torch.no_grad():
            ...
            def recalibration(train_features, test_features, threshold):
                ...
                train_cov_matrix = torch.matmul(
                    train_features_centered.T, train_features_centered
                ) / (train_features.size(0) - 1)
                train_cov_matrix += (
                    torch.eye(train_cov_matrix.size(0)).to(args.device) * 1e-6
                )
                ...
                for i, test_feature in enumerate(test_features):
                    ...
                    mahalanobis_distance = torch.sqrt(
                        torch.matmul(torch.matmul(diff.T, train_cov_matrix_inv), diff)
                    )
                    ...
                    if mahalanobis_distance > threshold:
                        recalibrated_test_features[i] = (
                            test_feature - train_mean + test_mean
                        )
                    else:
            ...
            recalibrated_test_features = recalibration(
                train_features, test_features, threshold
            )
            out = model.cls_head(recalibrated_test_features)

            test_acc = (
                (torch.argmax(out, dim=-1) == torch.argmax(test_y, dim=-1))
                .float()
                .mean()
            )
            ...
            print(f"epoch: {epoch} test acc {test_acc.item()}")
            print(
                f"epoch: {epoch}, preds {np.unique(list_of_preds, return_counts=True)}"
            )
            print("\n")
...
def draw_feature_difference(args, dataset, model):
    ...
    X = TSNE(n_components=2).fit_transform(
        np.concatenate([train_feat, test_feat], axis=0)
    )
    ...
    plt.scatter(
        X[: len(train_feat), 0],
        X[: len(train_feat), 1],
        c=np.argmax(dataset.train_y, axis=1),
        cmap="rainbow",
    )
    plt.subplot(2, 2, 2)
    plt.scatter(
        X[len(train_feat) :, 0],
        X[len(train_feat) :, 1],
        c=np.argmax(dataset.test_y, axis=1),
        cmap="rainbow",
    )
...
def using_shap_values(args, model, dataset):
    ...
    for idx_at_hand in recon_sorted_idx[:10]:
        ...
        mi_train = sklearn.feature_selection.mutual_info_regression(
            subset_train_x[:, mask], subset_train_x[:, idx_at_hand]
        )
        mi_test = sklearn.feature_selection.mutual_info_regression(
            subset_test_x[:, mask], subset_test_x[:, idx_at_hand]
        )
...
def train_graph_embedding(args, init_model, dataset):
    ...
    embed_vector_train = (
        torch.tensor(node2vec_train[idx_at_hand]).float().to(args.device)
    )
    ...
    with torch.no_grad():
        ...
        plt.subplot(2, 1, 1)
        plt.bar(
            range(len(embed_vector_train.detach().cpu())),
            embed_vector_train.detach().cpu(),
        )
        plt.subplot(2, 1, 2)
        plt.bar(
            range(len(embed_vector_test.detach().cpu())),
            embed_vector_test.detach().cpu(),
        )
        plt.show()
...
def train_gnn_bias(args, init_model, dataset):
    ...
    from sklearn.feature_selection import mutual_info_regression
    from utils.graph_embedding import (
        get_mi_matrix,
    ...
    for i in range(len(graph_train_input.nodes)):
        with torch.no_grad():
            ...
            graph_train_input.nodes[i]["x"] = torch.mean(
                init_model.get_feature(train_x_masked), dim=0
            )  # Replace num_features with the actual number of features per node
    ...
    for i in range(len(graph_test_input.nodes)):
        with torch.no_grad():
            ...
            graph_test_input.nodes[i]["x"] = torch.mean(
                init_model.get_feature(test_x_masked), dim=0
            )  # Replace num_features with the actual number of features per node
    ...
    gnn = GraphNet(num_features=feat_dim, num_classes=dataset.train_y.shape[1]).to(
        args.device
    )
    ...
    print(f"loss is {loss}")
    print(
        f"origial acc is : {torch.mean((torch.argmax(vanilla_out.detach(), dim=-1) == torch.argmax(test_y, dim=-1)).float())}"
    )
    print(
        f"acc is : {torch.mean((torch.argmax(out, dim=-1) == torch.argmax(test_y, dim=-1)).float())}"
    )
...
def main(args):
    ...
    if os.path.exists(os.path.join(dir_path, args.out_dir, "source_model.pth")):
        init_model.load_state_dict(
            torch.load(os.path.join(dir_path, args.out_dir, "source_model.pth"))
        )
